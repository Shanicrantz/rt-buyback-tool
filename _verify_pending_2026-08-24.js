export const meta = {
  name: 'rt-buyback-verify-outliers-2026-08-24',
...
function verifyPrompt(i) {
  return `You are re-verifying outliers from Rajdhani Telecom's weekly buyback refresh. TODAY is 2026-08-24.
...
function refutePrompt(i, v) {
  return `You are an adversarial REFUTER for Rajdhani Telecom. TODAY is 2026-08-24. A verifier just researched these items; assume they are wrong until their numbers survive your checks.
...
const results = await pipeline(
  ...
  (i) => agent(verifyPrompt(i), { label: `verify:${i}`, phase: 'Verify', schema: VERIFY_SCHEMA, model: 'sonnet', agentType: 'general-purpose' }),
  (v, i) => agent(refutePrompt(i, v), { label: `refute:${i}`, phase: 'Refute', schema: REFUTE_SCHEMA, model: 'sonnet', agentType: 'general-purpose' }),
)
