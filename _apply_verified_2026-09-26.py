...
HAND_CONFIRMED_PHANTOM_PREV = {'realme_p3_ultra_5g_8_512':
    'India line-up is 8/128, 8/256, 12/256 only (max 256GB) — Mobigyaan + Zee Biz launch coverage, hand-read 2026-09-21'}  # removed in v6.6
HAND_CONFIRMED_PHANTOM = {
    'oppo_a6s_8_256': 'OPPO India newsroom (hand-read 2026-09-26): A6s 5G India = 4GB+128GB Rs18,999 and 6GB+128GB Rs20,999 only, launched 2026-03-18',
    'oppo_a6s_8_128': 'OPPO India newsroom (hand-read 2026-09-26): A6s 5G India = 4GB+128GB Rs18,999 and 6GB+128GB Rs20,999 only, launched 2026-03-18',
}
...
for key, v in ver.items():
    ...
    bf = v.get('buyback_final')
    # same incoherent-spread filter as the weekly script: a competitor quote above 85% of resale is a 'Get Upto'
    # headline, not a price anyone runs a business on (2026-09-26: iPhone 17e 256 at 95%, Vivo Y51 Pro at 86%).
    if isinstance(bf, (int, float)) and 0 < bf <= rs * 0.85: e['buyback_market'] = r100(bf)
...
# hand-confirmed phantoms that were not themselves sent to verification are removed on the same evidence
for _k, _why in HAND_CONFIRMED_PHANTOM.items():
    if _k in ph and _k not in {r[0] for r in removed}:
        removed.append((_k, ph[_k].get('display_name'), _why))
