export const meta = {
  name: 'rt-buyback-verify-outliers-2026-09-21',
  ...
  properties: {
    ...
    items: { type: 'array', items: {
      ...
      properties: {
        ...
        launch_date_final: { type: ['string', 'null'] },
        confidence: { type: 'string', enum: ['high', 'medium', 'low'] },
        note: { type: 'string' },
      },
...
function verifyPrompt(i) {
  return `You are re-verifying outliers from Rajdhani Telecom's weekly buyback refresh. TODAY is 2026-09-21.
...
- kind=placeholder_resale : this entry was just gap-added with resale recorded as EXACTLY 80% of its new price. That is the gap-audit prompt's fallback for a phone too new to have a used market — but this one is age_days old (see the item) and DOES trade used, so the 80% figure is an unresearched guess, almost certainly too HIGH. Find the real local resale from scratch and ignore the stored resale entirely. A months-old budget phone typically resells well below 80% of new.

- kind=newmodel : this model is NOT in the DB yet (a gap exposed this week). Settle whether it is/was genuinely sold in India under that exact name and config (brand India store/newsroom, major India retailer), its official India new price, its India first-sale date (report as launch_date YYYY-MM-DD), and a REAL mint-used resale from datapoints. Never derive resale as a fixed fraction of new.
- Any item may carry a "context" field — read it first; it says exactly what is in doubt and why.
- kind=recheck : an anchor that previous research could not settle — read the item's "context" for exactly why. Treat every stored and prior figure as unproven, find independent datapoints (OLX sold-adjusted, Cashify refurb as upper bound, dealer listings) for the EXACT size/storage, and say plainly if none exist.
...
function refutePrompt(i, v) {
  return `You are an adversarial REFUTER for Rajdhani Telecom. TODAY is 2026-09-21. A verifier just researched these items; assume they are wrong until their numbers survive your checks.
...
For kind=newmodel use 'grant' to ADD it (exists_final='yes' on positive India evidence AND a triangulated resale_final you would stake money on; set launch_date_final), 'hold' when either is missing, 'remove' when it was never sold in India.
